...
import cv2
# 在main函數之前，添加代碼以讀取測試圖片和語句
test_image_path = '00005.png'
test_sentence = 'remove the bird'
...
def main(args):
    ...
    test_image = cv2.transform(test_image)

    # 將語句轉換為BERT輸入格式
    input_ids = tokenize.encode(test_sentence, return_tensors="pt")

    with torch.no_grad():
        output = language_model.generate(input_ids, max_length=50, num_return_sequences=1)
    description = tokenizers.decode(output[0], skip_special_tokens=True)
    
    # 使用模型進行預測
    with torch.no_grad():
        mask = model(test_image.unsqueeze(0).to(device), input_embedding.squeeze(0).to(device))
    ...
    # 在圖片上添加描述文字
    cv2.putText(test_image, description, (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
