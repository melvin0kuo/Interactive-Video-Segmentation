...
def evaluate(model, data_loader, args, bert_model, device, num_classes, epoch, logger, baseline_model):
    ...
    with torch.no_grad():
        for data in metric_logger.log_every(data_loader, 100, header):
            ...
            image, target, sentences, attentions = image.to(device), target.to(0), sentences.to(0), attentions.to(0)
...
def train_one_epoch(model, criterion, optimizer, data_loader, lr_scheduler, device, epoch, args, print_freq, logger,
    ...
    for data in metric_logger.log_every(data_loader, print_freq, header):
        ...
        image, target, sentences, attentions = data
        image, target, sentences, attentions = image.to(0), target.to(0), sentences.to(0), attentions.to(0)

        sentences = sentences.squeeze(1)   # (B, 10)
        attentions = attentions.squeeze(1)   # (B, 10)
        ...
        if args.baseline_bilstm:
            ...
            last_hidden_states = bert_model(sentences, attention_mask=attentions)[0]   # (B, 10, 768)
             
        embedding = last_hidden_states[:, 0, :]   # (B, 768)
        output, vis_emb, lan_emb = model(image, embedding.squeeze(1))   # vis_emb shape = (6, 256, 60, 60), lan_emb shape = (6, 256, 60, 60)
        # if iterations == 2800:
        #     pic_path = os.path.join('output_pics', str(epoch))
        #     os.mkdir(pic_path)
        #     out_mask = output['out'][2].cpu()
        #     out_fig = out_mask.argmax(0).data.numpy()
        #     # out_mask = output['out'][2]
        #     # out_fig = torch.argmax(out_mask.squeeze(), dim=0).detach().cpu().numpy()
        #     fig = plt.imshow(out_fig)
        #     plt.savefig(pic_path + '/out.png')
        #     gt = target[2].unsqueeze(2).cpu()
        #     gt_fig = gt.data.numpy()
        #     fig1 = plt.imshow(gt_fig)
        #     plt.savefig(pic_path + '/mask.png')

        loss = criterion(output, target, args)   # target shape = (B, 480, 480)
